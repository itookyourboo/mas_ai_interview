...
### Lightweight Hybrid Lexical Boost (Practical)

**What it is:** a cheap hybrid retrieval trick that combines dense similarity with simple lexical features (without full BM25 infra or cross-encoder rerank).

**Why it helps:** dense embeddings can return semantically close but wrong items for short queries ("Что такое FastAPI" -> "Что такое MRO"). A lexical boost prioritizes exact/similar title matches.

**Typical scoring (example):**

```text
rank = 0.50 * dense_score
     + 0.25 * qid_match
     + 0.10 * title_exact_or_soft_match
     + 0.10 * tag_overlap
     + 0.05 * token_overlap
```

Where:
- `dense_score` is normalized similarity in `[0,1]`
- `qid_match` indicates whether the answer belongs to a top candidate question
- `title_exact_or_soft_match` boosts exact/substring matches
- `tag_overlap` improves domain focus
- `token_overlap` is lightweight lexical overlap on query/title tokens

**Strict exact-match gate (recommended):**
- If exact question title is found in question index, return answers for this `question_id` first.
- Use broad semantic retrieval only as fallback when exact gate has no answers.

**When to use this pattern:**
- Local Chroma setup
- Short FAQ-style queries
- Limited resources (no expensive reranker)
- Need predictable behavior in interview/evaluation systems

**Ablation toggles (useful for experiments):**
- `RAG_HYBRID_LEXICAL_BOOST=1|0` — enables/disables lexical boost in reranking
- `RAG_STRICT_EXACT_GATE=1|0` — enables/disables strict exact-question gate before broad retrieval

This allows clean A/B comparisons in research:
- baseline dense-only (`0/0`)
- hybrid only (`1/0`)
- hybrid + strict gate (`1/1`)

### What "semantic score from embeddings" means

**Definition:** semantic score is a numeric estimate of meaning similarity between a query and a document vector.

**Pipeline:**
1. Convert query text to an embedding vector.
2. Compare with stored document vectors in vector DB.
3. Compute similarity (or distance converted to similarity).
4. Use the score for ranking candidates.

**Why it is useful:**
- Finds relevant content even when wording differs.
- Improves recall for paraphrases and domain terms.

**Common failure mode:**
- Short template-like queries ("Что такое X") can retrieve other "Что такое Y" items because global semantics look similar.
- This is why semantic score alone is often insufficient for FAQ/interview datasets.

**Practical fix (used in this project):**
- Keep semantic score as the main signal.
- Add lightweight lexical signals (exact/soft title match, token overlap, tag overlap).
- Apply strict exact-question gate when exact title match is found.

**Important implementation note:**
- Different backends can return either similarity or distance.
- Always normalize to a consistent `[0,1]` range before mixing with other ranking features.

**Quick interpretation table (project-level heuristic):**

| Normalized semantic score | Practical interpretation | Recommended action |
|---|---|---|
| `>= 0.80` | Strong semantic match | Keep as top candidate; likely relevant |
| `0.60 - 0.79` | Good but possibly broad match | Keep, but verify with lexical/tag signals |
| `0.45 - 0.59` | Weak/ambiguous match | Use reranking; prefer candidates with exact title or qid match |
| `< 0.45` | Likely noisy match | Usually drop unless no alternatives exist |

These thresholds are **empirical** and should be calibrated on your own benchmark set (`Hit@k`, `MRR`, expert relevance labels).
